...
export default function LoginPage() {
  ...
  const [mockMode, setMockMode] = useState(() => localStorage.getItem('admin_mock') === 'true');
  const { login } = useAuth();
  ...
  const toggleMock = () => {
    const next = !mockMode;
    setMockMode(next);
    localStorage.setItem('admin_mock', next ? 'true' : 'false');
  };
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', background: '#0f172a', fontFamily: lang === 'fa' ? 'system-ui, sans-serif' : 'inherit' }}>
      <div style={{
        ...
        <form onSubmit={handleSubmit} style={{
          ...
          <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
            <div style={{ fontSize: '0.8rem', color: '#6366f1', fontWeight: 600, letterSpacing: 2, marginBottom: 8, textTransform: 'uppercase' }}>
              WikiNazar
            </div>
          ...
          <div
            ...
            style={{
              ...
              marginBottom: '1.5rem', cursor: 'pointer', fontSize: '0.75rem', userSelect: 'none',
              color: mockMode ? '#4ade80' : '#64748b',
              background: mockMode ? 'rgba(74,222,128,0.1)' : 'rgba(100,116,139,0.1)',
              padding: '6px 12px', borderRadius: 8, border: `1px solid ${mockMode ? 'rgba(74,222,128,0.3)' : 'transparent'}`,
            }}
          >
            <span style={{
              width: 8, height: 8, borderRadius: '50%', display: 'inline-block',
              background: mockMode ? '#4ade80' : '#64748b',
            }} />
            {t.login.mockToggle.replace('{status}', mockMode ? t.login.on : t.login.off)}
          </div>
          ...
          {mockMode && (
            <div style={{
              marginTop: '1rem', textAlign: 'center', fontSize: '0.7rem', color: '#4ade80', opacity: 0.7,
            }}>
              {t.login.credentials}
            </div>
          )}
